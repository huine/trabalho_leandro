...
class Usuarios(SimpleItem):
    ...
    def cadastrar_usuario(self, email, senha, nome, uf, cidade, sexo, rua,
                          numero, data_nasc, bairro, complemento):

        try:
            id_usr = self._zsql_criar_usuario(
                email=email, senha=senha, nome_usuario=nome)[0]
        except:
            id_usr = None
        ...
        usr = self.get_by_id(id_usr['id_usuario'])

        try:
            insert = self._zsql_insere_dados_usr(
                id_usuario=id_usr['id_usuario'], uf=uf, cidade=cidade,
                sexo=sexo, rua=rua, numero=numero, dt_nascimento=data_nasc,
                bairro=bairro, complemento=complemento)[0]
        except:
            insert = 0
    ...
    _zsql_insere_dados_usr = SQL(
        id='zsql_insere_dados_usr', title='', connection_id='connection',
        arguments='id_usuario\nsexo\ndt_nascimento\nuf\ncidade\nrua\n' +
        'numero\nbairro\ncomplemento', template=open(
            product_path + 'sql/zsql_insere_dados_usr.sql').read()
    )

    _zsql_criar_usuario = SQL(
        id='zsql_criar_usuario', title='', connection_id='connection',
        arguments='email\nsenha\nnome_usuario', template=open(
            product_path + 'sql/zsql_criar_usuario.sql').read()
    )
...
class Usuario(SimpleItem):
    ...
    def __init__(self, id, email, tipo_usuario, nome_usuario, logado=False,
                 ip_login=""):
        self._id = id
        self._email = email
